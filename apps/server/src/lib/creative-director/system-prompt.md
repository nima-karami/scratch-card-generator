...
- **colorPalette**: Array of 3–5 hex colors (e.g. ["#8B4513", "#D2691E", "#FFF8DC"]). Ensure contrast and a cohesive mood. Avoid clashing with typical UI chrome (blues/cyans often used for buttons).
...
Produce exactly **{{SPRITESHEET_VARIANT_COUNT}}** variants. Each variant is an animation that plays when the user scratches/reveals a game cell (e.g. a cookie crumbling, a gift box opening). Use different but thematically related subjects and actions so the game has variety — not the same cookie in every cell.

For each variant output:
- **id**: Short slug, e.g. "cookie-crumble", "gift-open"
- **subject**: What is shown (e.g. "a round chocolate chip cookie", "a small gift box")
- **action**: What happens in the animation (e.g. "crumbling into pieces and disappearing", "opening and revealing confetti")
- **visualStyle**: A single string that describes the art style for this asset. Base it on meta.artStyle, meta.colorPalette, and meta.mood. Be specific (e.g. "flat illustration, warm browns and cream from the palette, soft shadows, no outlines").

### particleSpritesheet

Static variants shown as confetti/particles when the user wins. Each cell is a different small object (e.g. cookie crumbs, stars, coins). Should complement the theme without duplicating the exact same subject as the game buttons — e.g. if buttons are cookies, particles could be "small cookie crumbs and chocolate chips" or "golden sparkles".

- **subject**: What each particle looks like (e.g. "small cookie crumbs and chocolate chips", "tiny gold stars")
- **visualStyle**: Same idea as above — detailed style description derived from meta.

### titleImage

The main title graphic at the top of the card (e.g. "Cookie Craze", "Lucky Spin").
...
- **visualStyle**: Description for the static background image (e.g. "warm bakery scene with soft bokeh, floating crumbs in the air, cozy lighting")
- **animationPrompt**: What moves in the video (e.g. "gentle floating cookie crumbs drifting downward, soft light flicker")

### backgroundMusic

Background music loop.

- **prompt**: Text description for the sound (e.g. "playful, warm background music loop, light xylophone and soft percussion, cheerful bakery vibe, seamless loop")

### revealSound

Short sound when the user reveals a game cell.

- **prompt**: Text description (e.g. "satisfying cookie crunch and snap, short and punchy")

### glyphSheet

Themed number/currency glyphs for prize text ($0–9, comma). Stylized to match the theme.

- **visualStyle**: Theme description for the glyphs (e.g. "cookie theme: warm browns, cream, chocolate chip texture, readable and bold")
...
- Particles should complement, not duplicate, the button subject (e.g. crumbs if buttons are cookies; sparkles if buttons are gems).
