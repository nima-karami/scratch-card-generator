...
const USAGE = `
Usage: npm run generate-glyph-sheet -- --input <path> --visual-style "<style>" --output <path> [options]

Options:
  --input <path>       Path to the predefined glyph sheet image (12 glyphs: $ , 0-9 on solid background)
  --visual-style <text> Style for stylization. Same as Creative Director glyphSheet.visualStyle.
...
Example:
  npm run generate-glyph-sheet -- --input ./base-font.png --visual-style "cookie theme, warm browns, cream" --output ./glyph-cookies.png
  npm run generate-glyph-sheet -- --input ./base-font.png --visual-style "dinosaur theme" --output ./out.png --slice
`;
...
async function main(): Promise<void> {
  ...
  if (!inputPath || !visualStyle || !outputPath) {
    console.error("Error: --input, --visual-style, and --output are required.");
    console.error(USAGE);
    process.exit(1);
  }
  ...
  try {
    ...
    const result = await generateGlyphSheet(
      { baseImageBuffer, visualStyle, cols, rows },
      { slice, inputPath },
    );
    ...
    if (config.debug.glyphSheet) {
      try {
        ...
        await appendFile(
          ...
          `${new Date().toISOString()}\tvisualStyle="${visualStyle}"\tinput=${inputPath}\tsuccess=false\terror=${msg}\n`,
        );
      } catch {
        // ignore log failure
      }
